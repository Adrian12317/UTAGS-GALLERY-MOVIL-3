...
import { StyledContainer, StyledInputCreate } from "./styledPostsComponent";
...
export default function CreatePost({ setIsCreate, getItems }) {
  ...
  const handleSubmit = async () => {
    ...
    if (image) {
      ...
      let image_url_1 =
        "https://firebasestorage.googleapis.com/v0/b/login-adrian-44a42.appspot.com/o/posts%2F%20" +
        nameFile +
        ".jpg?alt=media";
    ...
    await axios
      .post("https://api-utagsgallery-codes.herokuapp.com/posts", request)
      .then(function ({ data }) {
        // handle success
  ...
  return (
    <>
      {!isPhoto ? (
        <StyledContainer>
          {isLoading ? (
            <ActivityIndicator size="large" color="#2F3676" />
          ) : (
            <Text></Text>
          )}
